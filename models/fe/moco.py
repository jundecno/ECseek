...
class MoCo(nn.Module):
    ...
    """
    def __init__(self, n_key_points, base_encoder, dim, K, m=0.999, T=0.07, mlp=False, use_cnn=False):
        """
    ...
    @torch.no_grad()
    def _batch_shuffle_ddp_cnn(self, ld, am):
        """
        Batch shuffle, for making use of BatchNorm.
        *** Only support DistributedDataParallel (DDP) model. ***
        """
        # gather from all gpus
        batch_size_this = am.shape[0]
        seq_len = am.shape[1]
        len_gather = concat_all_gather(torch.tensor(seq_len, dtype=torch.int).reshape(-1, 1).cuda())
        max_seq_len = int(len_gather.max())
        am_pad_row = torch.cat(
            (am, torch.zeros(batch_size_this, seq_len, max_seq_len - seq_len).cuda()), dim=2)
        am_pad = torch.cat(
            (am_pad_row, torch.zeros(batch_size_this, max_seq_len - seq_len, max_seq_len).cuda()), dim=1)

        ld_gather = concat_all_gather(ld)
        am_pad_gather = concat_all_gather(am_pad)
        batch_size_all = am_pad_gather.shape[0]
        ...
        return ld_gather[idx_this], am_pad_gather[idx_this], idx_unshuffle
    ...
    def forward(self, args, test=False):
        ...
        with torch.no_grad():  # no gradient to keys
            ...
            if self.use_cnn:
                ld_2, am_2, idx_unshuffle = self._batch_shuffle_ddp_cnn(ld_2, am_2)
            else:
