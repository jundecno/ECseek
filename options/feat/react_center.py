...
def get_frag_around_tetrahedral_center(mol, idx):
    ...
    return Chem.MolFragmentToSmiles(mol, ids_to_include, isomericSmiles=True, atomSymbols=symbols, allBondsExplicit=True, allHsExplicit=True)
...
def get_changed_atoms(reactants, products):
    ...
    for atom_tag, ar, ap in tetra_atoms:
        ...
        if atom_tag in changed_atom_tags:
            ...
            unchanged = check_tetrahedral_centers_equivalent(ar, ap) and ChiralType.CHI_UNSPECIFIED not in [ar.GetChiralTag(), ap.GetChiralTag()]
            if unchanged:
                if VERBOSE:
...
def extract_reacting_center(rxn, rxn_aam):
...
def cleaning_rxn(file_path, save_path):
    # *使用C代替，去掉氢气
    rxn_idx_dict = json_load(file_path)
    new_rxn_idx_dict = {}
    for rxn in rxn_idx_dict:
        new_rxn_idx_dict[rxn] = rxn.replace("*", "C")
    json_dump(save_path, new_rxn_idx_dict)


def calc_reacting_center(data_path, save_path):
    rxn2aam_dict = json_load(data_path)

    reacting_center_map = {}
    for rxn in tqdm(rxn2aam_dict):
        reacting_center_map[rxn] = extract_reacting_center(rxn, rxn2aam_dict[rxn]) # normalized rxn, reaction center
    json_dump(save_path, reacting_center_map)


if __name__ == "__main__":
    # cleaning_rxn(f"{root_path}/data/enzyme/RHEA/proc/rxn2smi.json", f"{root_path}/data/features/rxn2normal.json")
    ...
    calc_reacting_center(
        ...
        f"{root_path}/data/features/rxn2rc.json",
    )
