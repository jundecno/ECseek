...
def main_multiprocess(input_dir, pdb_fasta, pdb_redo_dir, n_blast=5, n_process=8):
    ...
    with ThreadPoolExecutor(max_workers=n_process) as executor:
        list(tqdm(executor.map(run_quiet, command_list), total=len(command_list), desc="Downloading items"))


if __name__ == "__main__":

    parser = argparse.ArgumentParser()
    parser.add_argument("--input_dir", type=str, help="Input directory containing CIF files", default="/data/zzjun/ECseek/data/Enzyme/ENZYME/afdb")
    parser.add_argument(
        "--output_dir", type=str, help="Output directory for AlphaFill transplantion results", default="/data/zzjun/ECseek/data/Enzyme/ENZYME/afilldb"
    )
    ...
    # python run_afill.py --input_dir /data/zzjun/ECseek/data/Enzyme/ENZYME/afdb --output_dir /data/zzjun/ECseek/data/Enzyme/ENZYME/afilldb  --pdb_fasta /data/zzjun/datasets/pdb-redo_seqs.fa --pdb_redo_dir /data/zzjun/datasets/pdb-redo
